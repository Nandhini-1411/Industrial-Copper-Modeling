...
def predict_status(country,item_type,order_day,order_month,order_year,delivery_year,delivery_month,delivery_day,
    ...
    with open(r"D:/CAPSTONE/COPPER/Class_model.pkl", "rb") as f:
    ...
    user_data = np.array([[country,item_type_encoded_value, order_year, order_month, order_day, delivery_year, 
                            delivery_month, delivery_day, application, width, product_ref, thickness_boxcox, quantity_tons_boxcox, selling_price]])
    y_pred = model_class.predict(user_data)
    return 1 if y_pred == 1 else 0

def predict_selling_price(country, status, item_type, item_date_input, delivery_year,
                          delivery_month, delivery_day, application, width, product_ref, 
    ...
    with open(r"D:/CAPSTONE/COPPER/Regres_Model.pkl", "rb") as f:
        model_regg = pickle.load(f)
    user_data = np.array([[country, status_encoded_value, item_type_encoded_value, order_year, order_month, order_day, delivery_year, 
...
if option == "PREDICT SELLING PRICE":
    ...
    with col1:
        ...
        application = st.number_input("Enter Application")
    ...
    with col3:
        if st.button("Predict Status"):
            status = predict_status(
                country, item_type, item_date_input.day, item_date_input.month, item_date_input.year, 
